...
# ALPACA landmark placement for all brain regions (HEADLESS)
# ==========================================================
...
# ----------------------------------------------------------
# SAFETY: Ensure we are inside 3D Slicer's environment
# ----------------------------------------------------------
try:
    import slicer
    import ALPACA
except ImportError:
    raise RuntimeError("This script must be run via Slicer's Python engine, not standard Python. Use: Slicer --no-splash --no-main-window --python-script run_alpaca_pipeline.py")
...
BASE = "/path/to/2_landmark_placement" # EDIT THIS PATH
OUTPUT_ROOT = os.path.join(BASE, "output")
CSV_OUT_DIR = os.path.join(BASE, "output")
os.makedirs(OUTPUT_ROOT, exist_ok=True)

# Set to None to auto-discover all region folders
REGIONS = None
...
if __name__ == "__main__":
    try:
        main()
        slicer.app.exit(0) # Success exit
    except Exception as e:
        import traceback
        traceback.print_exc()
        print(f"\n[ERROR] Pipeline crashed: {e}")
        slicer.app.exit(1) # Error exit